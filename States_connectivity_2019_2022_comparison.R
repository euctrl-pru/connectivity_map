...
ipairs2019<- read_csv(here("data", "ipairs33_v0_0_2019Q1.csv"))
ipairs2022<- read_csv(here("data", "ipairs33_v0_0_2022Q1.csv"))
...
ipair <- ipair_2019 %>%
  left_join(ipair_2022, by = c("rfrom","rto")) %>%
  mutate(diff = allFlights.y-allFlights.x) %>%
  # select(rfrom.x, rto, diff) %>%
  left_join(country_code, by = c("rfrom" = "country_iso2_code")) %>%
  arrange(desc(diff))
# %>%
# mutate(
...
data_for_map %>% 
  ...
  scale_fill_fermenter(
    name       = "flights choice in 2022 compared to 2019" ,
    # name       = NULL,
  ...
  labs(
    ...
    caption="Period: 07/03/22-13/03/22"
  ) +
  theme_map() +
